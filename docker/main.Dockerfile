...
RUN wget --no-verbose https://static.rust-lang.org/dist/rust-${RUST_VERSION}-x86_64-unknown-linux-gnu.tar.gz \
 && wget --no-verbose https://static.rust-lang.org/dist/rust-${RUST_VERSION}-x86_64-unknown-linux-gnu.tar.gz.asc \
 && gpg --verify rust-${RUST_VERSION}-x86_64-unknown-linux-gnu.tar.gz.asc rust-${RUST_VERSION}-x86_64-unknown-linux-gnu.tar.gz \
 && tar -xzf rust-${RUST_VERSION}-x86_64-unknown-linux-gnu.tar.gz \
 && ./rust-${RUST_VERSION}-x86_64-unknown-linux-gnu/install.sh \
 && rm -rf rust-${RUST_VERSION}-x86_64-unknown-linux-gnu.tar.gz rust-${RUST_VERSION}-x86_64-unknown-linux-gnu.tar.gz.asc rust-${RUST_VERSION}-x86_64-unknown-linux-gnu
...
RUN wget --no-verbose https://go.dev/dl/go${GO_VERSION}.linux-amd64.tar.gz \
 && echo "${GO_CHECKSUM} go${GO_VERSION}.linux-amd64.tar.gz" | sha256sum --check \
 && tar -C /usr/local --strip-components=1 -xzf go${GO_VERSION}.linux-amd64.tar.gz \
...
RUN wget --no-verbose https://nodejs.org/dist/v${NODE_VERSION}/node-v${NODE_VERSION}-linux-x64.tar.xz \
 && echo "${NODE_CHECKSUM} node-v${NODE_VERSION}-linux-x64.tar.xz" | sha256sum --check \
...
RUN wget --no-verbose https://www.php.net/distributions/php-${PHP_VERSION}.tar.gz \
 ...
 && cd php-${PHP_VERSION} && ./configure --enable-pcntl --enable-shmop --enable-sysvmsg --with-gmp && make -j && make install && cd .. \
 && rm -rf php-${PHP_VERSION}.tar.gz php-${PHP_VERSION}
...
RUN wget --no-verbose https://downloads.python.org/pypy/pypy${PYPY_PYTHON_VERSION}-v${PYPY_VERSION}-linux64.tar.bz2 \
 && echo "${PYPY_CHECKSUM} pypy${PYPY_PYTHON_VERSION}-v${PYPY_VERSION}-linux64.tar.bz2" | sha256sum --check \
 && tar -C /usr/local -xjf pypy${PYPY_PYTHON_VERSION}-v${PYPY_VERSION}-linux64.tar.bz2 \
 && ln -s /usr/local/pypy${PYPY_PYTHON_VERSION}-v${PYPY_VERSION}-linux64/bin/pypy3 /usr/local/bin/pypy3 \
 && rm pypy${PYPY_PYTHON_VERSION}-v${PYPY_VERSION}-linux64.tar.bz2
...
RUN wget --no-verbose https://www.lua.org/ftp/lua-${LUA_VERSION}.tar.gz \
 && echo "${LUA_CHECKSUM} lua-${LUA_VERSION}.tar.gz" | sha256sum --check \
 && tar -xzf lua-${LUA_VERSION}.tar.gz \
 && cd lua-${LUA_VERSION} && make -j && make install && cd .. \
