...
def main(args: argparse.Namespace) -> None:
    for filename in args.filenames:
        ...
        if len(lines) == args.iterations:
            ...
            continue
        print(
            f"{filename}: consolidating {len(lines)} lines into {args.iterations} lines"
        )
        ...
        for i in range(args.iterations):
            ...
            for j in range(1, len(lines) // args.iterations):
                entry = merge_entries(
                    entry, data[i * (len(lines) // args.iterations) + j]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--iterations", type=int, required=True, help="number of iterations performed"
    )
